...
function SimulacrosList() {
  ...
  const onPublish = async (id) => {
    ...
    try {
      ...
      setData((prev) => prev.map((item) => (item._id === id ? response.simulacro : item)));
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              data.map((simulacro) => {
                const inAction = actionId === simulacro._id;
                return (
                  <tr key={simulacro._id} className="border-b">
                    ...
                    <td className="p-3">
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => navigate(`${simulacro._id}`)} className="bg-blue-600 text-white px-3 py-1 rounded" disabled={inAction}>Ver</button>
                        <button onClick={() => navigate(`${simulacro._id}/resultados`)} className="bg-indigo-600 text-white px-3 py-1 rounded" disabled={inAction}>Resultados</button>
                        {simulacro.estado === 'borrador' ? (
                          <>
                            <button onClick={() => onPublish(simulacro._id)} className="bg-emerald-600 text-white px-3 py-1 rounded" disabled={inAction}>Publicar</button>
                            <button onClick={() => onDelete(simulacro._id)} className="bg-red-600 text-white px-3 py-1 rounded" disabled={inAction}>Eliminar</button>
                          </>
                        ) : null}
                      </div>
                    </td>
